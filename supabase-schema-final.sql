...
create table if not exists profiles (
  ...
  full_name text,
  avatar_url text,
  role text default 'member' check (role in ('member', 'moderator', 'admin', 'super_admin')),
  bio text,
  phone text,
...
drop policy if exists "Authenticated can submit" on listings;
create policy "Authenticated can submit" on listings for insert with check (auth.uid() is not null);
drop policy if exists "Owner can update" on listings;
...
drop policy if exists "Users can edit own review" on reviews;
...
drop policy if exists "Chat messages are public" on chat_messages;
...
drop policy if exists "Auth users can react" on reactions;
create policy "Auth users can react" on reactions for insert with check (auth.uid() = user_id);
drop policy if exists "Users delete own reaction" on reactions;
create policy "Users delete own reaction" on reactions for delete using (auth.uid() = user_id);

-- Bookmarks: self only
drop policy if exists "Users see own bookmarks" on bookmarks;
...
drop policy if exists "Users see own claims" on listing_claims;
create policy "Users see own claims" on listing_claims for select using (auth.uid() = user_id);
drop policy if exists "Auth users can claim" on listing_claims;
create policy "Auth users can claim" on listing_claims for insert with check (auth.uid() = user_id);
drop policy if exists "Admins see all claims" on listing_claims;
...
drop trigger if exists on_claim_status_change on listing_claims;
...
-- ============================================================
drop policy if exists "Verified owner can update their listing" on listings;
...
drop policy if exists "Admins view bulk imports" on bulk_imports;
...
drop policy if exists "Auth users can RSVP" on event_rsvps;
...
create index if not exists posts_event_start_idx on posts(event_start) where type = 'event' and status = 'published';
...
drop policy if exists "Users see own points" on user_points;
create policy "Users see own points" on user_points for select using (auth.uid() = user_id);
drop policy if exists "Anyone insert points" on user_points;
...
create index if not exists user_points_time_idx on user_points(created_at desc);
...
drop trigger if exists on_points_change on user_points;
...
drop policy if exists "Admins see all sessions" on user_sessions;
create policy "Admins see all sessions" on user_sessions for select using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin', 'moderator'))
);
drop policy if exists "Anyone insert session" on user_sessions;
...
-- ============================================================
create table if not exists power_areas (
  id uuid default uuid_generate_v4() primary key,
...
create table if not exists power_cut_reports (
  id uuid default uuid_generate_v4() primary key,
...
drop policy if exists "Price reports readable" on price_reports;
create policy "Price reports readable" on price_reports for select using (true);
drop policy if exists "Anyone can submit price" on price_reports;
create policy "Anyone can submit price" on price_reports for insert with check (true);

drop policy if exists "Power reports readable" on power_cut_reports;
create policy "Power reports readable" on power_cut_reports for select using (true);
drop policy if exists "Anyone can report power" on power_cut_reports;
...
drop policy if exists "Admin manages price reports" on price_reports;
create policy "Admin manages price reports" on price_reports for all using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);
drop policy if exists "Admin manages power reports" on power_cut_reports;
create policy "Admin manages power reports" on power_cut_reports for all using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);

-- Indexes for performance
create index if not exists price_reports_item_time_idx on price_reports(item_id, reported_at desc);
create index if not exists power_reports_area_time_idx on power_cut_reports(area_id, reported_at desc);
create index if not exists price_reports_ip_idx        on price_reports(reporter_ip, reported_at desc);
create index if not exists power_reports_ip_idx        on power_cut_reports(reporter_ip, reported_at desc);
...
drop policy if exists "Fuel reports public" on fuel_reports;
create policy "Fuel reports public" on fuel_reports for select using (true);
drop policy if exists "Anyone can report fuel" on fuel_reports;
...
drop policy if exists "Lost found public" on lost_found;
create policy "Lost found public" on lost_found for select using (status != 'expired');
drop policy if exists "Anyone can post" on lost_found;
create policy "Anyone can post" on lost_found for insert with check (true);
drop policy if exists "Poster can update" on lost_found;
create policy "Poster can update" on lost_found for update using (
  auth.uid() = poster_id or
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);

create index if not exists lost_found_type_idx on lost_found(type, status, reported_at desc);
create index if not exists lost_found_cat_idx  on lost_found(category, status);
...
drop policy if exists "Auth can post jobs" on jobs;
create policy "Auth can post jobs" on jobs for insert with check (auth.uid() = poster_id);
drop policy if exists "Poster can manage" on jobs;
...
create index if not exists jobs_cat_idx    on jobs(category, status);
...
drop policy if exists "Approved notices public" on notices;
create policy "Approved notices public" on notices for select using (status = 'approved');
drop policy if exists "Anyone can submit notice" on notices;
create policy "Anyone can submit notice" on notices for insert with check (true);
drop policy if exists "Admin manages notices" on notices;
...
drop policy if exists "Departures public" on bus_departures;
create policy "Departures public" on bus_departures for select using (departure_date >= current_date - 1);
drop policy if exists "Anyone report departure" on bus_departures;
create policy "Anyone report departure" on bus_departures for insert with check (true);

create index if not exists bus_dep_route_date_idx on bus_departures(route_id, departure_date, departure_time);
...
drop policy if exists "Weather alerts public" on weather_alerts;
create policy "Weather alerts public" on weather_alerts for select using (status = 'active');
drop policy if exists "Anyone can report weather" on weather_alerts;
create policy "Anyone can report weather" on weather_alerts for insert with check (true);
drop policy if exists "Admin manages alerts" on weather_alerts;
...
drop trigger if exists on_tour_review on tour_reviews;
...
drop policy if exists "Verified prices public" on verified_prices;
create policy "Verified prices public" on verified_prices for select using (true);
drop policy if exists "Admin verifies prices" on verified_prices;
...
drop policy if exists "Admin deletes notices" on notices;
...
drop policy if exists "Admin deletes donations" on donations;
...
drop policy if exists "Health services public" on health_services;
create policy "Health services public" on health_services for select using (status = 'active');
drop policy if exists "Anyone can post health" on health_services;
create policy "Anyone can post health" on health_services for insert with check (true);
drop policy if exists "Admin manages health" on health_services;
create policy "Admin manages health" on health_services for all using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);

create index if not exists health_type_idx on health_services(type, status, start_date);

-- ============================================================
-- ADMIN MODERATION — Universal delete policies
-- ============================================================
-- Allow admin/mod to delete from all community tables
drop policy if exists "Admin deletes lost_found" on lost_found;
...
drop policy if exists "Admin deletes price_reports" on price_reports;
create policy "Admin deletes price_reports" on price_reports for delete using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);
drop policy if exists "Admin deletes power_reports" on power_cut_reports;
create policy "Admin deletes power_reports" on power_cut_reports for delete using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);
drop policy if exists "Admin deletes fuel_reports" on fuel_reports;
create policy "Admin deletes fuel_reports" on fuel_reports for delete using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);
drop policy if exists "Admin deletes chat" on chat_messages;
create policy "Admin deletes chat" on chat_messages for delete using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);
drop policy if exists "Admin deletes health" on health_services;
create policy "Admin deletes health" on health_services for delete using (
  exists (select 1 from profiles where id = auth.uid() and role in ('admin','moderator'))
);
drop policy if exists "Admin deletes weather" on weather_alerts;
...
drop policy if exists "Markets public" on markets;
create policy "Markets public"       on markets for select using (true);
drop policy if exists "Admin manages markets" on markets;
...
drop policy if exists "Emergency contacts public" on emergency_contacts;
create policy "Emergency contacts public" on emergency_contacts
  for select using (is_active = true);
drop policy if exists "Admin manages emergency" on emergency_contacts;
...
drop policy if exists "Votes public read" on listing_votes;
create policy "Votes public read"  on listing_votes for select using (true);
drop policy if exists "Members can vote" on listing_votes;
create policy "Members can vote"   on listing_votes for insert
  with check (auth.uid() = user_id);
drop policy if exists "Members delete vote" on listing_votes;
...
drop trigger if exists on_listing_vote on listing_votes;
...
drop policy if exists "Super admin full access listings" on listings;
